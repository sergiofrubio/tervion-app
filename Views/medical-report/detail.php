...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('btnDownloadReportPdf')?.addEventListener('click', function() {
        ...
        if (!element || typeof html2pdf === 'undefined') {
            window.location.href = '<?= PROJECT_ROOT ?>/historial/pdf?id=<?= urlencode($report['historial_id']) ?>';
            return;
        }

        const opt = {
            margin: 0,
            filename: 'Informe_Clinico_<?= preg_replace('/[^A-Za-z0-9_-]/', '_', $report['paciente_nombre'] . '_' . $report['paciente_apellidos']) ?>_<?= htmlspecialchars($report['historial_id']) ?>.pdf',

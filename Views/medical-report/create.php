...
<!-- SCRIPT DE INICIALIZACIÓN DE TINYMCE CON HERRAMIENTAS COMPLETAS TIPO WRITER Y EXPORTACIÓN PDF -->
<script>
document.addEventListener('DOMContentLoaded', function() {
    
    // 1. INICIALIZACIÓN DE TINYMCE PARA CADA SECCIÓN DEL FOLIO
    ...
    // 2. INSERCIÓN DE SNIPPETS CLÍNICOS RÁPIDOS EN LOS EDITORES
    ...
    document.getElementById('btnDownloadReportPdf')?.addEventListener('click', function() {
        // Asegurar sincronización de contenido antes de exportar
        if (typeof tinymce !== 'undefined') {
            tinymce.triggerSave();
        }
        ...
        const opt = {
            margin: 0,
            filename: 'Informe_Clinico_<?= preg_replace('/[^A-Za-z0-9_-]/', '_', ($paciente['nombre'] ?? '') . '_' . ($paciente['apellidos'] ?? '')) ?>_<?= htmlspecialchars($paciente['usuario_id']) ?>.pdf',
    ...
    // 6. GUARDADO DEL FORMULARIO CLÍNICO
    document.getElementById('btnSubmitReportStudio')?.addEventListener('click', function() {
        ...
        const diagEditor = tinymce.get('diag_editor');
        const tratEditor = tinymce.get('trat_editor');
        const obsEditor = tinymce.get('obs_editor');
        ...
        // Asignar a los campos hidden del formulario

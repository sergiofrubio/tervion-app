...
<head>
  ...
  <title>Error del Servidor - Tervion</title>
...
<body class="min-h-screen flex flex-col justify-between font-sans antialiased text-gray-900 bg-cover bg-center bg-no-repeat <?= $hasSystemAlert ? 'pt-7' : '' ?>" style="background-image: url('<?= PROJECT_ROOT ?>/public/img/fondo.jpg');">
  ...
  <?php if (file_exists(TEMPLATE_DIR . 'system-alert.php')) { include TEMPLATE_DIR . 'system-alert.php'; } ?>
  ...
  <!-- Main Error Card Section -->
  <main class="flex-grow flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative">
    <div class="w-full max-w-md p-8 bg-white/90 backdrop-blur-md rounded-3xl shadow-2xl border border-white/40 text-center animate-fade-in-up">

      <!-- Icon illustration -->
      <div class="mx-auto mb-6 w-20 h-20 rounded-3xl bg-rose-50 border border-rose-100 flex items-center justify-center text-rose-500 shadow-inner">
        <i class="bi bi-shield-exclamation text-4xl"></i>
      </div>

      <!-- Error Message -->
      <span class="inline-block px-3 py-1 mb-3 text-[11px] font-bold uppercase tracking-wider text-rose-700 bg-rose-50 rounded-full border border-rose-200">
        Error 500
      </span>
      <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight text-gray-900">Algo ha ido mal</h1>
      <p class="mt-3 text-sm text-gray-600 leading-relaxed">
        Ha ocurrido un problema interno en el servidor mientras procesábamos tu solicitud. Nuestro equipo técnico ha sido notificado para resolverlo cuanto antes.
      </p>

      <!-- Buttons -->
      <div class="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
        <a href="#" onclick="window.location.reload();" class="inline-flex items-center justify-center gap-2 rounded-xl bg-gray-950 px-5 py-3 text-sm font-semibold text-white shadow-md hover:bg-gray-800 transition-all hover:scale-105 active:scale-95 cursor-pointer">
